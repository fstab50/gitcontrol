...
.. code:: bash
    ...
    $ giteven --profile default --operation up          # rotate (change out) keys


List keys for the default awscli user: ::

    $ giteven --profile default --operation list        # list key information
...
**Q**: I have 2 access key sets associated with my IAM user. Which set will ``giteven`` replace when I rotate access keys?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


**A**: In short, ``giteven`` compares the age of the two keysets in your account
...
.. code:: bash
    ...
    $ giteven --profile myIAMuser --user-name JoeUser --operation up
...
**Q**: How do I configure logging for ``giteven``?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**A**: Logging is turned off by default for a new installation of ``giteven``.
...
**Q**: Why doesn't ``giteven`` produce any log output?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. code:: json
        ...
        "LOGGING": {
            ...
            "LOG_MODE": "FILE",
            "LOG_PATH": "/home/blake/logs/giteven.log"
        }
    }
...
.. code:: bash
    ...
    # giteven auto rotate
    0 7 * * *  giteven --profile johndoe --operation up --auto


The ``--auto`` switch supresses all output to stdout, so ``giteven`` produces no output
...
.. code:: bash
    ...
    # giteven auto rotate
    0 7 * * 5  giteven --profile myUser --operation up --auto
...
.. code:: bash
    ...
    # weekly
    0 10 * * 6  KEYUP=$(which giteven);  $KEYUP --profile johndoe --operation up --auto
...
**A**: ``giteven`` has special logic to avoid disrupting STS Temporary Credentials if they
       are present in your local awscli configuration.  The following summarizes the
       logic ``giteven`` employs when dealing with temporary credentials:

        - ``giteven`` detects when STS credentials are present in the local credentials files

        - If temporary credentials are detected, ``giteven`` attempts to determine if
        ...
        - **If authentication fails**:
          ``giteven`` assumes the temporary credentials are expired and ignores them.
          key rotation proceeds automatically as normal

        - **If authentication succeeds**:
          ``giteven`` assumes the temporary credentials are active and prohibits
          access key rotation.
...
**A**: Call the ``keyconfig`` script.
    ...
    You can display ``giteven``'s local configuration file via by calling the console script:
